...
class ProductManager {
    ...
    getProductById = async (idProduct) => {
        try {
            const fileData = await fs.promises.readFile(this.pathFile, 'utf-8');
            const data = JSON.parse(fileData);
            const product = data.findIndex((prod) => prod.id === parseInt(inProduct));
    ...
    setProductById = async (idProduct, updateProduct) => {
        try {
            const fileData = await fs.promises.readFile(this.pathFile, 'utf-8');
            const data = JSON.parse(fileData);
    ...
    deleteProductById = async (idProduct) => {
        try {
            const fileData = await fs.promises.readFile(this.pathFile, 'utf-8');
            const data = JSON.parse(lileData);
            const productIndex = data.findIndex((prod) => prod.id === parseInt(inProduct));
